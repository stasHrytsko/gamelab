...
describe('укладка (§4–5)', () => {
  ...
  it('поражение no_moves: оставшееся число никуда не помещается', () => {
    // Четвёрка по диагонали рвёт поле на куски из 3 и 4 клеток — семёрке некуда встать.
    const out = place(start(), 1, [[0, 1], [1, 1], [1, 2], [2, 2]]);
    expect(out.valid).toBe(true);
    expect(out.state.status).toBe('failed');
  });

  it('уровень уже не решить, но ходы есть — игра продолжается (решаемость наперёд не проверяется)', () => {
    const s = createState({ id: 8, map: ['......', '......'], numbers: [4, 4, 4] });
    // Квадрат 2×2 отрезает слева кусок из 2 клеток: собрать поле уже нельзя, но четвёрка справа помещается.
    const out = place(s, 0, [[0, 1], [0, 2], [1, 1], [1, 2]]);
    expect(out.state.status).toBe('playing');
    expect(solve(out.state.board, [4, 4])).toBeNull();
  });
});
