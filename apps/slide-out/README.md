...
2. **Root Directory: `apps/slide-out`.** Остальное Vercel возьмёт из
...
- `src/ui/board.ts` — поле: плитка едет за пальцем до соседней клетки,
  упирается в занятую, жест короче 18 px — тап.
- `src/ui/game.ts` — игровой экран, строка целей, попапы.
- Лог §8 пишется только на устройство: `localStorage['slide-out:log']`.
