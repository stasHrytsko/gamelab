import type { BlockColor } from '../engine/types.ts';

const svg = (body: string, cls = ''): string =>
  `<svg class="${cls}" viewBox="0 0 24 24" aria-hidden="true">${body}</svg>`;

export const GLYPH: Readonly<Record<BlockColor, string>> = {
  red: '<circle cx="12" cy="12" r="9" fill="currentColor"/>',
  blue: '<rect x="3.5" y="3.5" width="17" height="17" rx="3.5" fill="currentColor"/>',
  yellow: '<path d="M12 3.2c.7 0 1.3.4 1.7 1l8 14c.8 1.4-.2 3-1.7 3H4c-1.5 0-2.5-1.6-1.7-3l8-14c.4-.6 1-1 1.7-1z" fill="currentColor"/>',
};

export const glyph = (color: BlockColor, cls = 'glyph'): string => svg(GLYPH[color], cls);
...
export const icon = {
  ...
  goal: svg('<circle cx="12" cy="12" r="8" fill="none" stroke="currentColor" stroke-width="3"/><circle cx="12" cy="12" r="3" fill="currentColor"/>'),
...
export function colorTile(color: BlockColor, extra = ''): string {
