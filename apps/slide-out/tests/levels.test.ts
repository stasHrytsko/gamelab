...
/** Группы появления целей из спеки §6, по unlockAfterServed. */
...
describe('пакет уровней', () => {
  test('пять уровней: 3, 4, 5, 7, 8 целей', () => {
    expect(LEVEL_COUNT).toBe(5);
    expect(LEVELS.map((level) => level.goals.length)).toEqual([3, 4, 5, 7, 8]);
  });

  test('цели появляются группами как в спеке: 1→1→1, 1→1→2, 1→2→2, 1→2→2→2, 1→2→2→3', () => {
    expect(LEVELS.map((level) => arrivalGroups(level.goals.map((p) => p.unlockAfterServed)))).toEqual([
      [1, 1, 1],
      [1, 1, 2],
      [1, 2, 2],
      [1, 2, 2, 2],
  ...
  test.each(LEVELS.slice(1).map((level) => [level.id, level] as const))(
    'уровень %i не проходится, если двигать только цвета ждущих целей (kill-критерий)',
    (_id, level) => {
